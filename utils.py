...
def parse_lib_time(val):
    ...
    if isinstance(val, (int, float)):
        ...
        # 先转为带北京时区的对象，再去掉 tzinfo 方便与本地计算
...
def smart_refresh_logic(jsid, ic, token, accNo):
    ...
    # 场景 3: 刷新进行中预约
        ...
        headers = {"Cookie": f"JSESSIONID={jsid}; ic-cookie={ic}", "token": token, "Content-Type": "application/json;charset=UTF-8"}
        ...
        for res in r8452:
            ...
            print(f"[Smart] 刷新预约: 原定结束时间 {old_end_dt.strftime('%Y-%m-%d %H:%M:%S')} (北京时间)")
